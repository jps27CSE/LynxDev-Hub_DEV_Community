...
> **Date:** 2026-09-22
> **Status:** Task 6 — Done (Build Mode, uncommitted) — Strict Review pending 2026-09-24
> **Scope:** Expand `Admin Panel > Users` — list usernames + safe admin actions
> **Progress:** ✅ Tasks 1-6 complete 2026-09-22/24 — Tasks 7-11 pending
...
*   **2026-09-24 Task 6 Done:** Created `app/admin/users/_components/UsersTable.tsx:1` — `"use client"` `useRouter`+`useTransition`+`useState(searchInput)` synced `currentQ`, debounced 300ms `setTimeout` → `router.push(buildHref({q,page:1}))`, `Select` for `ADMIN_USERS_SORTS` whitelisted, `Table`/`Badge`/`Input`+`Search` icon, pagination `Button`+`ChevronLeft/Right` with `hasMore`/`page<=1` disabled + `isPending` dim, preserves `subscription` via `currentSubscription`. Created `lib/admin-users-constants.ts:1` (client-safe `PAGE_SIZE=20`, `MAX_SEARCH_LENGTH=100`, `MAX_PAGE=500`, `AdminUsersSort`, `ADMIN_USERS_SORTS`) to fix `Client Component Browser` bundling `config/db.tsx`→`mysql2`→`tls`/`node:async_hooks` when `UsersTable` imported `lib/admin-users.ts` (server-only `db`). Refactored `app/admin/users/page.tsx:1` to Server boundary delegating to `UsersTable` (removed inline table). Verified `tsc --noEmit` ✅ `build` ✅ (`ƒ /admin/users` 34/34) 4.8s. Uncommitted.
