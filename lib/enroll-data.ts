...
async function fetchEnrollments(userId: number): Promise<EnrolledCourse[]> {
  const rows = await db
    .select({
  ...
  return rows.map((r) => {
    const rawProgress = r.enrollment.progress as Partial<
...
export const getEnrollmentsByClerkId = cache(
  async (clerkId: string): Promise<EnrolledCourse[]> => {
    try {
